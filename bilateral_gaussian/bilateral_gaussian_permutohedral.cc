  ...
  void Compute(OpKernelContext* context) override {
    ...
    OP_REQUIRES(context, TensorShapeUtils::IsMatrixOrHigher(image.shape()),
                errors::InvalidArgument("image must be 3-D, but got shape ",
                                        image.shape().DebugString())); //TODO:jzuern: does not cover all cases
    ...
    */
    ...
    // convert input and output image to EIGEN TensorMap
    auto input_eigen = image.tensor<float,3>();
    ...
    PermutohedralLattice::filter(input_eigen, positions_eigen, &out_eigen);
...
#define REGISTER(Dev, T, Index)                   \
  REGISTER_KERNEL_BUILDER(                        \
      Name("BilateralGaussianPermutohedral") \
          .Device(DEVICE_##Dev)                   \
          .TypeConstraint<T>("T")                 \
          .TypeConstraint<Index>("T"),      \
      BilateralGaussianPermutohedralOp<Dev##Device, T, Index>);
REGISTER(CPU, float, float)
REGISTER(CPU, double, double)
REGISTER(CPU, Eigen::half, double)
