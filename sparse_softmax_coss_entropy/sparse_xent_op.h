...
template <typename T, typename Index>
class SparseXentLossGenerator_jzuern {
 public:
  EIGEN_DEVICE_FUNC EIGEN_ALWAYS_INLINE SparseXentLossGenerator_jzuern(
      typename TTypes<const T, 2>::Tensor32Bit logits,
      typename TTypes<const T, 1>::Tensor32Bit sum_exp_logits,
      typename TTypes<const Index, 1>::Tensor32Bit labels,
      typename TTypes<const T, 1>::Tensor32Bit weights,
      const Index max_depth)
      : logits_(logits),
        sum_exp_logits_(sum_exp_logits),
        labels_(labels),
  ...
  operator()(const Eigen::array<int, 2>& coords) const {
    ...
    printf("depth = %i\n", depth);
    printf("label = %i\n", label);
    printf("sum_exp_logits_(batch) = %f\n", sum_exp_logits_(batch));
    printf("logits_(coords) = %f\n", logits_(coords));
    printf("weights_(label) = %f\n\n", weights_(label));

    if (label == depth){
          printf("return value without class weight: %f\n", Eigen::numext::log(sum_exp_logits_(batch)) - logits_(coords));

          printf("return value with class weight: %f\n\n\n\n\n", (Eigen::numext::log(sum_exp_logits_(batch)) - logits_(coords))* weights_(label) );
    }
 ...
 private:
  ...
  typename TTypes<const T, 1>::Tensor32Bit sum_exp_logits_;
  typename TTypes<const Index, 1>::Tensor32Bit labels_;
  typename TTypes<const T, 1>::Tensor32Bit weights_;
...
namespace functor {
  ...
  // Functor used by SparseXentOp to do the computations.
  template <typename Device, typename T, typename Index>
  struct SparseXentFunctor_jzuern {
    // Computes Cross Entropy loss and backprop.
    //
    // logits: batch_size, num_classes.
    // labels: num_classes.
...
// specializations for both device types.
template <typename Device, typename T, typename Index>
struct SparseXentEigenImpl_jzuern {
  static void Compute(const Device& d, typename TTypes<T>::ConstMatrix logits,
                      typename TTypes<Index>::ConstVec labels,
                      typename TTypes<T>::ConstVec weights,
                      typename TTypes<T>::Vec scratch,
    ...
    printf("SparseXentEigenImpl_jzuern: void Compute(...)\n");
...
// the resulting value to all classes.
    ...
    //     ((logits - max_logits) - log(sum(exp(logits - max_logits)))))
    //  along classes
    generator::SparseXentLossGenerator_jzuern<T, Index> sparse_xent_loss_gen(
        sparse_xent_helpers::To32BitConst<T>(backprop),
        sparse_xent_helpers::To32BitConst<T>(scratch),
        To32Bit(labels),
        To32Bit(weights), // TODO(jzuern): shouldnt this be To32BitConst ?????
        backprop.dimension(1) /* max_depth */);
